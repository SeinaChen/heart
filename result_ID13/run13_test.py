...
import numpy as np
#Document_ls = ['step']
Document_ls = ['heart13_test','sleep13_test','step13_test']
...
def processing(df,date,a,b):
  ...
  day_time = [(i[1],i[2]) for i in new if i[0] == "2018-12-"+ date]
...
date_ls.extend([str(i+1) for i in range(9,15)])
...
for i in range(15):
  for t in range(96):
     string = "2018-12-"+str(i+1)+" "+str(0.25*t)+"時"
     datestr_ls.append(string)

print(len(datestr_ls))
#'heart','sleep','step'
...
data.drop(data.index[(data.sleep == 3) & (data.step == 0)], inplace=True)
...
data.to_csv("/Users/xinnanchen/Desktop/datascience/result_ID13/result13_test.csv")
